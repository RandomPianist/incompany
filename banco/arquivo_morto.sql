...
CREATE TABLE log2 AS (
	...
	JOIN (
		...
		WHERE tabela NOT IN ('retiradas', 'estoque', 'solicitacoes', 'setores', 'atribuicoes') AND origem <> 'SYS'
	...
	UNION ALL (
		...
		WHERE tabela IN ('retiradas', 'estoque', 'solicitacoes', 'setores', 'atribuicoes') OR origem = 'SYS'
	)
);
DROP TABLE log;
